...
namespace Eto
{
	...
	public abstract class Widget : IHandlerSource, IDisposable, ICallbackSource
	{
		...
		protected Widget()
		{
			...
			if (platform == null)
				throw new Exception(string.Format(CultureInfo.CurrentCulture, "Platform instance is null. Have you created your application?"));
			var info = platform.FindHandler(GetType());
			if (info == null)
				throw new Exception(string.Format(CultureInfo.CurrentCulture, "Type for '{0}' could not be found in this platform", GetType().FullName));
