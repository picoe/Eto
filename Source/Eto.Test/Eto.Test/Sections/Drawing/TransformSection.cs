...
namespace Eto.Test.Sections.Drawing
{
	public class TransformSection : Scrollable
	{
		...
		void MatrixTests(Graphics g)
		{
			...
			{
				g.SaveTransform();
				g.MultiplyTransform(Matrix.FromScaleAt(new SizeF(1, -1), imageScaleLocation + image.Size / 2, generator: Platform));
			...
			// test skewing
			{
				...
				g.DrawLine(Pens.White(Platform), 110, textSize.Height + 2, 110 + textSize.Width, textSize.Height + 2);
		...
		void DirectTests(Graphics g)
		{
			...
			// test skewing
			g.SaveTransform();
			g.MultiplyTransform(Matrix.FromSkew(20, 20, Platform));
			var textSize = g.MeasureString(font, "Skewed Text");
			g.DrawText(font, Colors.White, new PointF(110, 0), "Skewed Text");
			g.DrawLine(Pens.White(Platform), 110, textSize.Height + 2, 110 + textSize.Width, textSize.Height + 2);
		...
		{
			var pen = new Pen(color, generator: Platform);
		...
		void DrawRotatedArcs(Graphics g, Color color, PointF center, Action<PointF, float> action)
		{
			for (float i = 0; i <= 360f; i += 90f)
			{
				g.SaveTransform();
				action(center, i);
				g.DrawArc(new Pen(color, generator: Platform), RectangleF.FromCenter(center, new SizeF(50, 50)), 0, 45f);
				g.RestoreTransform();
			}
		}
	}
