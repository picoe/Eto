...
namespace Eto.GtkSharp.Forms.Printing
{
	public class PrintDocumentHandler : WidgetHandler<Gtk.PrintOperation, PrintDocument>, IPrintDocument
	{
		...
		public override void AttachEvent (string id)
		{
			...
			case PrintDocument.PrintPageEvent:
				Control.DrawPage += (o, args) => {
					using (var graphics = new Graphics(Widget.Platform, new GraphicsHandler(args.Context.CairoContext, args.Context.CreatePangoContext ()))) {
		...
		public PrintSettings PrintSettings {
			get {
				if (settings == null)
					settings = Control.PrintSettings.ToEto (Control.DefaultPageSetup, false, Widget.Platform);
				return settings;
			}
			set {
				settings = value;
