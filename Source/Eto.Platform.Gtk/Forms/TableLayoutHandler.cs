...
namespace Eto.Platform.GtkSharp
{
	public class TableLayoutHandler : GtkLayout<Gtk.Table, TableLayout>, ITableLayout
	{
		...
		{
			if (controls.GetLength(1) == 1) return Gtk.AttachOptions.Expand|Gtk.AttachOptions.Shrink|Gtk.AttachOptions.Fill;
			else if (columnOptions.ContainsKey(column)) return columnOptions[column];
			else if (columnOptions.Count == 0 && column == controls.GetLength(1)-1) return Gtk.AttachOptions.Expand|Gtk.AttachOptions.Shrink|Gtk.AttachOptions.Fill;
			else return Gtk.AttachOptions.Fill;
		}

		private Gtk.AttachOptions GetRowOptions(int row)
		{
			if (controls.GetLength(0) == 1) return Gtk.AttachOptions.Expand|Gtk.AttachOptions.Shrink|Gtk.AttachOptions.Fill;
			else if (rowOptions.ContainsKey(row)) return rowOptions[row];
			else if (rowOptions.Count == 0 && row == controls.GetLength(0)-1) return Gtk.AttachOptions.Expand|Gtk.AttachOptions.Shrink|Gtk.AttachOptions.Fill;
			else return Gtk.AttachOptions.Fill;
		}
		...
		bool Attach(Control child, int x, int y)
		{
			...
			if (old != null && old != child) {
				if (old != null) Control.Remove((Gtk.Widget)old.ControlObject);
			}
			...
			if (child != null)
			{
				controls[y, x] = child;
				var widget = (Gtk.Widget)child.ControlObject;
				if (widget.Parent is Gtk.Container) ((Gtk.Container)widget.Parent).Remove(widget); 
