...
{
	public class TableLayoutHandler : iosLayout<UIView, TableLayout>, ITableLayout
	{
		Control[,] views;
		bool[] xscaling;
		bool[] yscaling;
		...
		void Layout()
		{
			...
			for (int y=0; y<heights.Length; y++) {
				...
				for (int x=0; x<widths.Length; x++)
				{
					...
					var view = views[y, x];
					if (view != null && view.Visible) {
						var nsview = view.ControlObject as UIView;
		...
		public void Add(Control child, int x, int y)
		{
			...
			if (current != null) {
				var currentView = (UIView)current.ControlObject;
				if (currentView != null) currentView.RemoveFromSuperview();	
			}
			var view = (UIView)child.ControlObject;
			views[y, x] = child;
			Layout();
