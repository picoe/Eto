...
public static class GirControlExtensions
{
	public static Gtk.Widget? GetContainerWidget(this Control control)
	{
		if (control == null)
			return null;
		var containerHandler = control.Handler as IGirControl;
	...
	public static IGirControl? GetGtkControlHandler(this Control control)
	{
		if (control == null)
			return null;
		var containerHandler = control.Handler as IGirControl;
...
static class GtkControl
{
	...
	public static HashSet<IGirControl> EnteredControls = new HashSet<IGirControl>();
	public static bool ShouldCaptureMouse;
}

public abstract class GirControl<TControl, TWidget, TCallback> : WidgetHandler<TControl, TWidget, TCallback>, Control.IHandler, IGirControl
	...
	where TCallback : Control.ICallback
{
	...
	public override IntPtr NativeHandle => throw new NotImplementedException(); // Control.Handle;
	...
	{
		get => ContainerControl.Visible ? ContainerControl.GetAllocation() : UserPreferredSize;
		set
		{
	...
	protected virtual Color DefaultBackgroundColor
	{
		...
		// get => ContainerContentControl.GetBackground();
	}
	...
	protected new GirControlConnector Connector { get { return (GirControlConnector)base.Connector; } }

	protected override WeakConnector CreateConnector()
	{
		return new GirControlConnector();
	}

	/// <summary>
	/// Connector for events to keep a weak reference to allow gtk controls to be garbage collected when no longer referenced
	/// </summary>
	protected class GirControlConnector : WeakConnector
	{
		...
		new GirControl<TControl, TWidget, TCallback> Handler => (GirControl<TControl, TWidget, TCallback>)base.Handler;
...
/*
	...
	{
		get => throw new NotImplementedException();  //Widget.Properties.Get<Font>(GtkControl.Font_Key) ?? Widget.Properties.Get(GtkControl.Font_Key, FontControl.GetFont().ToEto());
		set
		{
			throw new NotImplementedException();
			/*
	...
	{
		if (!(Control.GetRoot() as Gtk.Widget).TranslateCoordinates(
				Control,
				point.X,
				point.Y,
				out var widgetX,
				out var widgetY))
			return point;
		return new PointF((float)widgetX, (float)widgetY);
	}

	public PointF PointToScreen(PointF point)
	{
		if (!Control.TranslateCoordinates(
				(Gtk.Widget)Control.GetRoot(),
				point.X,
				point.Y,
				out var rootX,
				out var rootY))
		...
		return new PointF((float)rootX, (float)rootY);
	}
	...
	public Window GetNativeParentWindow() => throw new NotImplementedException(); //(Control.Toplevel as Gtk.Window).ToEtoWindow();
	...
	{
		get => throw new NotImplementedException();  //Widget.Properties.Get(GtkControl.IsMouseCaptured_Key, false) || EventControl.HasGrab;
		private set => Widget.Properties.Set(GtkControl.IsMouseCaptured_Key, value);
	}

	Control IGirControl.Widget => Widget;

	Control.ICallback IGirControl.Callback => Callback;

	public void TriggerMouseEnterIfNeeded() => throw new NotImplementedException();  // Connector.TriggerMouseEnterIfNeeded();
	public void TriggerMouseLeaveIfNeeded() => throw new NotImplementedException();  // Connector.TriggerMouseLeaveIfNeeded();
