...
namespace Eto.WinRT.Forms
{
	...
	public class TableLayoutHandler : WpfLayout<swc.Grid, TableLayout, TableLayout.ICallback>, TableLayout.IHandler
	{
		...
		public override wf.Size GetPreferredSize(wf.Size constraint)
		{
			...
			return new wf.Size(widths.Sum() + border.Padding.Horizontal(), height + border.Padding.Vertical());
		}
		...
		class MyEmptyCell : sw.FrameworkElement
		{
		}

		sw.FrameworkElement EmptyCell(int x, int y)
		{
			var empty = new MyEmptyCell();
		...
		void SetChildrenSizes()
		{
			...
			for (int y = 0; y < Control.RowDefinitions.Count; y++)
			{
				...
				for (int x = 0; x < Control.ColumnDefinitions.Count; x++)
				{
					...
					{
						var margin = childControl.ContainerControl.Margin;
						childControl.ParentMinimumSize = new wf.Size(Math.Max(0, maxx - margin.Horizontal()), Math.Max(0, maxy - margin.Vertical()));
					}
				}
			}
		}
		...
		sw.GridLength GetColumnWidth(int column)
		{
			...
			return new sw.GridLength(1, scale ? sw.GridUnitType.Star : sw.GridUnitType.Auto);
		}
