		...
		if (RowDefinitions.Count > 1 && (double.IsInfinity(constraint.Height) || !IsLoaded))
		{
			...
			for (int i = 0; i < RowDefinitions.Count; i++)
			{
				...
				if (rd.Height.IsStar)
				{
					...
					{
						var child = (mux.FrameworkElement)Children[childIndex];
...
public class TableLayoutHandler : WinUILayout<EtoGrid, TableLayout, TableLayout.ICallback>, TableLayout.IHandler
{
	...
	{
		Control = new EtoGrid { };
		border = new muc.Border();//new EtoBorder { Handler = this };
		border.Padding = Padding.Empty.ToWinUI();
	}

	public Size Adjust { get; set; }

	public override mux.FrameworkElement ContainerControl
	{
		get { return border; }
	}
	...
	public override Color BackgroundColor
	{
		...
		set { border.Background = value.ToWinUIBrush(); }
	}
	...
	public void CreateControl(int cols, int rows)
	{
		...
		for (int i = 0; i < cols; i++) Control.ColumnDefinitions.Add(new muc.ColumnDefinition());
		for (int i = 0; i < rows; i++) Control.RowDefinitions.Add(new muc.RowDefinition());
	...
	void Control_LayoutUpdated(object sender, object e) => SetChildrenSizes(false);

	mux.FrameworkElement EmptyCell(int x, int y)
	{
		var empty = new muc.Border();
		muc.Grid.SetColumn(empty, x);
		muc.Grid.SetRow(empty, y);
		return empty;
	}
	...
	void SetScale(bool force = false)
	{
		...
		//	{
		//		var handler = controls[x, y].GetFrameworkElement();
		//		if (handler != null)
	...
	void SetChildrenSizes(bool force)
	{
		...
		for (int y = 0; y < Control.RowDefinitions.Count; y++)
		{
			...
			for (int x = 0; x < Control.ColumnDefinitions.Count; x++)
			{
				...
				var child = controls[x, y];
				//var childControl = child.GetFrameworkElement();
				//if (childControl != null)
				//{
	...
	void SetMargins(bool force = false)
	{
		if (Widget.Loaded || force)
		{
			foreach (var child in Control.Children.OfType<mux.FrameworkElement>())
			{
				var x = muc.Grid.GetColumn(child);
				var y = muc.Grid.GetRow(child);
				SetMargins(child, x, y);
			}
		}
	}

	mux.GridLength GetColumnWidth(int column)
	{
		var scale = columnScale[column] || column == lastColumnScale;
		return scale ? new mux.GridLength(1, mux.GridUnitType.Star) : mux.GridLength.Auto;
	}

	mux.GridLength GetRowHeight(int row)
	{
	...
	void SetMargins(mux.FrameworkElement c, int x, int y)
	{
		c.HorizontalAlignment = mux.HorizontalAlignment.Stretch;
		c.VerticalAlignment = mux.VerticalAlignment.Stretch;
		var margin = new mux.Thickness();
	...
	public Padding Padding
	{
		...
		set { border.Padding = value.ToWinUI(); }
	}
	...
	public void Add(Control child, int x, int y)
	{
		...
		if (child != null)
		{
			var handler = child.GetFrameworkElement();
			var control = handler.ContainerControl;
			controls[x, y] = child;
			control.SetValue(muc.Grid.ColumnProperty, x);
			control.SetValue(muc.Grid.RowProperty, y);
			if (Widget.Loaded)
			{
	...
	public void Move(Control child, int x, int y)
	{
		var handler = child.GetFrameworkElement();
		var control = handler.ContainerControl;
		var oldx = muc.Grid.GetColumn(control);
		var oldy = muc.Grid.GetRow(control);
